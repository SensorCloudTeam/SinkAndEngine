...
void ComposeBasicData(char *sBasicData)
{
		...
		for(i=0; i<m_nDataCount; i++)
		{
			...
			strcat(sBasicData,"</Value><Cycle>");
			sprintf(sCycle,"%f",sensorCycle[c_pData.m_nMeasurementID-1]);
			strcat(sBasicData,sCycle);
			strcat(sBasicData,"</Cycle>\r\n");
		}

		get_Re(Res);
		strcat(sBasicData,"\t\t<Result type=\"Number\">");
		strcat(sBasicData,Res);
		strcat(sBasicData,"</Result>\r\n");
